...
func initPrecompiledContract() {

	contracts := map[string]string{
		"election":   "0x0000000000000000000000000000000000000009",
		"supervisor": "0x0000000000000000000000000000000000000008",
	}

	reg := func(name string, addr string) {
		contract := &models.Account{}
		contract, err := contract.Get(addr)
		if err != nil {
			contract = &models.Account{
				Address:        addr,
				Balance:        "0",
			...
			if err = contract.Insert(); err != nil {
				msg := fmt.Sprintf("Failed to insert %s contract account: %s", name, err.Error())
				panic(msg)
			}
		} else if contract.ContractName == "" {
			contract.ContractName = "election"
			if err = contract.Update(); err != nil {
				msg := fmt.Sprintf("Failed to update %s contract account: %s", name, err.Error())
				panic(msg)
			}
		}
	}
